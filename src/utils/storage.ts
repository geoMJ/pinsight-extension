...
export const getValueFromStorage = async (key: keyof ExtensionStorage): Promise<ExtensionOptions | null> => {
    const extensionStorage: ExtensionStorageRoot = await browser.storage.local.get(EXTENSION_STORAGE_KEY) || {};
...
export const setValueInStorage = async (key: keyof ExtensionStorage, value: any) => {
    const currentStorage: ExtensionStorageRoot = await browser.storage.local.get(EXTENSION_STORAGE_KEY) || {};
